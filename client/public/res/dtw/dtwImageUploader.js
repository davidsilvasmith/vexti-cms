define([
    ...
    function doImageInsert() {
        ...
        var fd = new FormData(document.getElementById("uploadFormInsert"));
        //fd.append("label", "WEBUPLOAD");
        $.ajax({
            url: "/system/upload",
            type: "POST",
            ...
            console.log("Server Output:");
            var cutoff = data.indexOf('/');
            data = data.substr(cutoff, data.length);
            console.log(data);
            $('#imageInsertPath').text(data);
        });
        return false;
    }
    ...
    dtwImageUploader.onReady = function() {
        ...
        $('.upload-local-image').click(doImageInsert);
